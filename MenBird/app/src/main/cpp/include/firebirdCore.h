    ...
    class Core {
        ...
        private:
            ...
            void _loadLibrary()
            {
                ...
                //dlopen("/data/data/com.example.menbird/files/firebird/lib/libfbclient.so.2", RTLD_NOW);
                //dlopen("/data/data/com.example.menbird/files/firebird/lib/libicudata.so.52", RTLD_NOW);
                //dlopen("/data/data/com.example.menbird/files/firebird/lib/libicuuc.so.52", RTLD_NOW);
                //dlopen("/data/data/com.example.menbird/files/firebird/lib/libicui18n.so.52", RTLD_NOW);
                // /data/data/com.example.menbird/files/firebird/UDF/
                if (!(_handle = dlopen("libFreeAdhocUDF.so", RTLD_NOW))) {
                    __android_log_print(ANDROID_LOG_ERROR, "LOADLIB", "Error when load lib freeHadoc : %s", dlerror());
                    //throw std::runtime_error("Error loading Firebird client library.");
                }
                else
                {
                    __android_log_print(ANDROID_LOG_ERROR, "LOADLIB", "Load succes");
                }
