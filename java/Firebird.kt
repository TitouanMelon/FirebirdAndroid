 ...
 */
class Firebird(baseContext : ContextWrapper) {
    ...
    private var SQL_TYPE : Map<String, Int> = mapOf(
        "MY_NULL" to -1,
        "SQL_TEXT" to 452,
        "SQL_VARYING" to 448,
        "SQL_SHORT" to 500,
        ...
        "SQL_DOUBLE" to 480,
        "SQL_D_FLOAT" to 530,
        "SQL_TIMESTAMP" to 510,
        "SQL_BLOB" to 520,
        "SQL_ARRAY" to 540,
        "SQL_QUAD" to 550,
        "SQL_TYPE_TIME" to 560,
        "SQL_TYPE_DATE" to 570,
        "SQL_INT64" to 580,
        "SQL_TIMESTAMP_TZ_EX" to 32748,
        "SQL_TIME_TZ_EX" to 32750,
        "SQL_INT128" to 32752,
        "SQL_TIMESTAMP_TZ" to 32754,
        "SQL_TIME_TZ" to 32756,
        "SQL_DEC16" to 32760,
        "SQL_DEC34" to 32762,
        "SQL_BOOLEAN" to 32764,
        "SQL_NULL" to 32766,
    )
     ...
     */
    fun connectJava(dbName : String = "DB.FDB", user : String = "SYSDBA", pass : String = "masterkey", useUsersFolders : Boolean = true) {
        if (useUsersFolders) // db is in /storage/emulated/0/Android/data/<packageName>/files folder and can be accessible by the user
                ...
                for (i in 1 until dataString.size) {
                    ...
                    for (j in 0 until dataString[i].size) {
                        ...
                        map[dataString[0][j][0]] = when (type) {
                            SQL_TYPE["MY_NULL"] as Int ->  {""}
                            SQL_TYPE["SQL_TEXT"] as Int -> {dataString[i][j][0].replace("\r\n", "")}
                            SQL_TYPE["SQL_VARYING"] as Int -> {dataString[i][j][0]}
                            SQL_TYPE["SQL_SHORT"] as Int -> {dataString[i][j][0].toInt()}
                            SQL_TYPE["SQL_LONG"] as Int -> {dataString[i][j][0].toLong()}
                            SQL_TYPE["SQL_FLOAT"] as Int -> {dataString[i][j][0].toFloat()}
                            SQL_TYPE["SQL_DOUBLE"] as Int -> {dataString[i][j][0].toDouble()}
                            SQL_TYPE["SQL_TIMESTAMP"] as Int -> {dataString[i][j][0]}
                            SQL_TYPE["SQL_BLOB"] as Int -> {dataString[i][j][0].replace("\r\n", "")}
                            SQL_TYPE["SQL_ARRAY"] as Int-> {dataString[i][j][0].replace("\r\n", "")}
                            SQL_TYPE["SQL_TYPE_TIME"] as Int -> {dataString[i][j][0]}
                            SQL_TYPE["SQL_TYPE_DATE"] as Int -> {dataString[i][j][0]}
                            SQL_TYPE["SQL_INT64"] as Int -> {dataString[i][j][0].toLong()}
                            SQL_TYPE["SQL_TIMESTAMP_TZ"] as Int -> {dataString[i][j][0]} //timestamp with timezone
                            SQL_TYPE["SQL_BOOLEAN"] as Int -> {dataString[i][j][0].toBoolean()}
